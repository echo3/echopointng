...
/** 
 * <code>ExpandableDelegate</code> 
 */

public class ExpandableDelegate extends AbstractComponentDelegate implements Expandable {

	
	/**
	 * Constructs a <code>ExpandableDelegate</code>
	 *
	 * @param delegatee
	 */
	public ExpandableDelegate(Component delegatee) {
		super(delegatee);
		setExpansionModel(new DefaultExpansionModel());
		if (! (delegatee instanceof Expandable))
			throw new IllegalArgumentException("The ExpandableDelegate delegatee component must implement Expandable");
	}
	...
	public void setExpansionGroup(ExpansionGroup newExpansionGroup) {
		...
		if (oldValue != null) {
			oldValue.removeExpandable((Expandable) getDelegatee());
		}
		if (newExpansionGroup != null) {
			newExpansionGroup.addExpandable((Expandable) getDelegatee());
		}
